...
contract TokenRoot is ITokenRoot {
    ...
    constructor() public {
        // How the tvm.pubkey() is set?
        require(tvm.pubkey() != 0, TokenRootErrors.error_tvm_pubkey_not_set);
        tvm.accept();
    ...
    modifier onlyOwner() {
        require(
            ...
            TokenRootErrors.error_message_sender_is_not_my_owner
        );
        _;
    }

    function deployWallet(uint256 _wallet_public_key, uint128 _deploy_ever) private view returns (address) {
        ...
        TvmCell dataInit = tvm.buildDataInit({
            ...
            contr: TokenWallet,
            varInit: {root_address: address(this), wallet_code: wallet_code}
        });
        TvmCell stateInit = tvm.buildStateInit(wallet_code, dataInit);
        ...
        address wallet = new TokenWallet{
            stateInit: stateInit,
            value: _deploy_ever, //the amount of native coins we are sending with the message
            wid: address(this).wid,
            flag: 0
        }(); //this flag denotes that we are paying for the creation of the message from the value we are sending with the contract.
        ...
        returns (address)
    {
        ...
        require(_wallet_public_key != 0, TokenRootErrors.error_deploy_wallet_pubkey_not_set);
        require(_deploy_evers >= 0.05 ton, TokenRootErrors.error_deploy_ever_to_small);
    ...
    function mint(address _to, uint128 _tokens) external override onlyOwner {
        ...
        require(
            address(this).balance > start_gas_balance,
            TokenRootErrors.error_insufficient_evers_on_contract_balance
        );
        require(_tokens > 0);
        ...
        ITokenWallet(_to).accept(_tokens);
    ...
    function deployWalletWithBalance(
        ...
        require(_wallet_public_key != 0, TokenRootErrors.error_deploy_wallet_pubkey_not_set);
        require(_deploy_evers >= 0.1 ton, TokenRootErrors.error_deploy_ever_to_small);

        // Similar fool-proof mechanism to the one above,
        // but here we also add _deploy_evers
        require(
            address(this).balance > start_gas_balance + _deploy_evers,
            TokenRootErrors.error_insufficient_evers_on_contract_balance
        );
        ...
        ITokenWallet(deployed_contract).accept(_tokens);
    ...
    onBounce(TvmSlice slice) external {
        ...
        if (functionId == tvm.functionId(ITokenWallet.accept)) {
